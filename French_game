...
# ==========================================
# 1. ONTARIO GRADE 2 FRENCH IMMERSION VOCABULARY LIST (100+ Words)
# ==========================================
VOCABULARY = [
    # School & Classroom
...
def load_mastery_data():
    """Load persistent mastery scores from JSON file."""
...
def save_mastery_data(data):
    """Save mastery scores to JSON file."""
    with open(MASTERY_FILE, "w") as f:
        json.dump(data, f)

def get_valid_question():
    """Select a word that hasn't been mastered (<5) and hasn't been played this session."""
    ...
    # Get all English definitions except the correct one
...
if 'initialized' not in st.session_state:
    ...
    st.session_state.session_history = [] # List of French words used this game
    st.session_state.game_over = False
    st.session_state.current_q = None
    st.session_state.feedback = None # To store "Correct" or "Wrong" message
...
st.markdown("For Grade 2 French Immersion")
...
# --- ACTIVE GAME SCREEN ---
    ...
    st.warning("Wow! You have mastered all the words in this list! (Score 5+ on everything). Ask your parent to add more words to the code!")
        ...
        for i, option in enumerate(q["options"]):
            with cols[i % 2]:
                if st.button(option, use_container_width=True):
                    ...
                    # 1. Update Session History (so it doesn't repeat this game)
                    ...
                    if option == correct_en:
                        ...
                        # 3. Update Persistent Mastery (Increment score)
                        ...
                        # Set feedback (No mastery increase for wrong answers)
...
# Footer
st.write("---")
st.caption("Keep practicing to unlock Mastery status for each word!")
